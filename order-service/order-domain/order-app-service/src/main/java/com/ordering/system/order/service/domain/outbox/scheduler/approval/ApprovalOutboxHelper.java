package com.ordering.system.order.service.domain.outbox.scheduler.approval;
...
import com.ordering.system.order.service.domain.ports.output.repository.ApprovalOutboxRepository;
...
public class ApprovalOutboxHelper {
    ...
    public void save(OrderApprovalOutboxMessage orderApprovalOutboxMessage) {
        ...
        if (response == null) {
            log.error("Could not save OrderApprovalOutboxMessage with outboxId: {}",
                    orderApprovalOutboxMessage.getId());
            throw new OrderDomainException("Could not save OrderApprovalOutboxMessage with outboxId: " +
                    orderApprovalOutboxMessage.getId());
        }
        log.info("OrderApprovalOutboxMessage saved with outboxId: {}", orderApprovalOutboxMessage.getId());
    }
    ...
    private String createPayload(OrderApprovalEventPayload orderApprovalEventPayload) {
        try {
            ...
            log.error("Could not create OrderApprovalEventPayload for orderId: {}",
                    orderApprovalEventPayload.getOrderId(), e);
            throw new OrderDomainException("Could not create OrderApprovalEventPayload for orderId: " +
                    orderApprovalEventPayload.getOrderId(), e);
        }
    }
