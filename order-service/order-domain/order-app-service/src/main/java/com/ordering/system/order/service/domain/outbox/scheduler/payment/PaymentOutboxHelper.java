package com.ordering.system.order.service.domain.outbox.scheduler.payment;
...
import com.ordering.system.order.service.domain.ports.output.repository.PaymentOutboxRepository;
...
public class PaymentOutboxHelper {
    ...
    public Optional<OrderPaymentOutboxMessage> getPaymentOutboxMessageBySagaIdAndSagaStatus(
            ...
            SagaStatus... sagaStatus) {
        return paymentOutboxRepository
                .findByTypeAndSagaIdAndSagaStatus(ORDER_SAGA_NAME, sagaId, sagaStatus);
    }
    ...
    public void save(OrderPaymentOutboxMessage orderPaymentOutboxMessage) {
        ...
        if (response == null) {
            log.error("Could not save OrderPaymentOutboxMessage with outboxId: {}", orderPaymentOutboxMessage.getId());
            throw new OrderDomainException("Could not save OrderPaymentOutboxMessage with outboxId: " +
                    orderPaymentOutboxMessage.getId());
        }
        log.info("OrderPaymentOutboxMessage saved with outboxId: {}", orderPaymentOutboxMessage.getId());
    }
    ...
    private String createPayload(OrderPaymentEventPayload paymentEventPayload) {
        try {
            ...
            log.error("Could not create OrderPaymentEventPayload object for orderId: {}",
                    paymentEventPayload.getOrderId(), e);
            throw new OrderDomainException("Could not create OrderPaymentEventPayload object for orderId: " +
                    paymentEventPayload.getOrderId(), e);
        }
    }
